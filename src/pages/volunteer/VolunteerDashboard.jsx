        ...
        {filtered.map(ngo => (
          <div key={ngo.id} className="glass-card" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
              ...
              {ngo.verified && <span className="badge badge-secondary" style={{ flexShrink: 0, marginLeft: '0.5rem' }}>Verified</span>}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.85rem', color: 'var(--color-text-secondary)', marginBottom: '0.35rem' }}>
              <Target size={14} /> {ngo.domain}
            </div>
            ...
            <div style={{ marginTop: 'auto', paddingTop: '1rem', borderTop: '1px solid var(--color-border)', display: 'flex', gap: '0.5rem' }}>
              <button className="btn btn-secondary" style={{ flex: 1 }}>Connect</button>
      ...
      <div className="space-y-4">
        {[
          ...
          <div key={i} className="list-item">
            <div>
              <p style={{ fontWeight: 600, color: 'var(--color-text-primary)', fontSize: '0.9rem', margin: 0 }}>{act.event}</p>
              <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', margin: 0 }}>{act.ngo} · {act.date}</p>
            </div>
            <span className="badge badge-primary">+{act.hours} hrs</span>
          </div>
        ))}
      </div>
    </div>
...
const VolunteerDashboard = () => {
  ...
  const tabs = [
    { id: 'directory', label: 'NGO Directory & Matching' },
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p>Make an impact, <span style={{ fontWeight: 600, color: 'var(--color-text-primary)' }}>{user?.name}</span></p>
      </div>
